...
void fillArray(const char* element, T* array, const int size, const T minValue, const T maxValue) {
    for (int i = 0; i < size; ++i) {
        do cout << element << i + 1 << COLON << SPACE;
        while (!tryGetValue(array[i], minValue, maxValue, ERROR_INCORRECT_VALUE));
    }
}

template<typename T>
void printArray(const T* array, const int size, bool isReverse) {
    if (isReverse) {
...
template<typename T>
void printArray(const char* element, const T* array, const int size, bool isReverse) {
    if (isReverse) {
