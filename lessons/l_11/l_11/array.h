...
template<typename T>
void fillArray(const char* element, const T* array, const int size) {
...
template<typename T>
void printArray(const T* array, const int size, bool isReverse = false) {
    if (isReverse) {
...
template<typename T>
void printExceptValue(const T* array, const int size, const T exeptValue) {
    bool first = true;
    for (int i = 0; i < size; ++i) {
...
template<typename T>
void printArray(const char* element, const T* array, const int size, bool isReverse = false) {
    if (isReverse) {
        for (int i = size - 1; i >= 0; --i) cout << element << i + 1 << COLON << SPACE << array[i] << endl;
    }
    else {
        for (int i = 0; i < size; ++i) cout << element << i + 1 << COLON << SPACE << array[i] << endl;
    }
}

template<typename T>
T getMinValue(const T* array, const int size) {
    T min = *array;
...
template<typename T>
T getMaxValue(const T* array, const int size) {
    T max = *array;
...
template<typename T>
T getSum(const T* array, const int size) {
    T sum = 0;
...
template<typename T>
float getAverage(const T* array, const int size) {
	return static_cast<float>(getSum(array, size)) / size;
}

template<typename T>
T* extend(const T* array, const int size) {
...
template<typename T>
bool contains(const T* array, const int size, const T value) {
    for (int i = 0; i < size; ++i) {
...
template<typename T>
int addValue(T* array, int& capacity, const T newValue) {
...
template<typename T>
int addValue(const char* msg, const T*& array, int& capacity) {
    T newValue = 0;
